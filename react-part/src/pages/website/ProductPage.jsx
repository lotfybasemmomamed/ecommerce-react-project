...
function ProductPage() {
  ...
  useEffect(() => {
    ...
    getProductById(id)
      .then((res) => {
        console.log("res", res);
        setSingleProductDetails(res.data[0]);
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  // get images products
  useEffect(() => {
    if (singleProductDetails?.images) {
      const formattedImages = singleProductDetails.images.map((img) => ({
        original: img.image,
        thumbnail: img.image,
      }));
      setImagesProduct(formattedImages);
    }
  }, [singleProductDetails]);

  // const getImagesProduct = (singleProductDetails) => {
  //   const formattedImages = singleProductDetails?.images?.map((img) => ({
  //     original: img.image,
  //     thumbnail: img.image,
  //   }));
        ...
        <>
          <div className="max-w-6xl lg:mt-[20px] mx-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div className="rounded-2xl shadow-lg overflow-hidden">
              <ImageGallery items={imagesProduct} showPlayButton={false} />
            </div>
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <p className="text-sm text-gray-500">
                  ({singleProductDetails?.ratings_number} reviews)
                </p>
              </div>
                ...
                <div className="flex items-center gap-3">
                  <span className="text-3xl font-extrabold text-blue-600">
                    ...
                    {(
                      ...
                      singleProductDetails?.price *
                        (singleProductDetails?.discount / 100)
                    ).toFixed(2)}
                  </span>
                  <span className="text-md text-gray-400 line-through">
                    ${Number(singleProductDetails?.price).toFixed(2)}
                  </span>
                </div>
